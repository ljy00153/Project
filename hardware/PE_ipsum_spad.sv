...
logic [`PSUM_SIZE-1:0] ipsum_spad [0:`OPSUM_SPAD_LEN-1];
...
assign ipsum_ready = (cs != ADD_IPSUM)? 1'b1 : 1'b0;//load ipsum into spad at anytime except ADD_IPSUM
...
always_ff @( posedge clk or posedge rst ) begin 
    integer i;
    if (rst) for (i = 0; i < 12; i = i + 1) ipsum_spad[i] <= 32'b0;
    else begin
        if (ipsum_valid) begin
            ipsum_spad[ofmap_count] <= ipsum;
        end
        else begin
            ipsum_spad[ofmap_count] <= ipsum_spad[ofmap_count];
        end 
    end
end
