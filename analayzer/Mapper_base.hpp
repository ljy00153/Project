...
class EyerissMapper_base
{
    public:
                    ...
                    { 
                        for (int N = tn; N <= 1024; N++) 
                        {
                            int used_bytes =  M * K * IFMAP_PER_PE * 4 
                                            + K * IFMAP_PER_PE * N * WEIGHT_PER_PE * 4 * 4
        ...
        virtual void generate_assembly(const string& base_file_path, const string& output_file_path)
        {
            ...
            int dram_ifmap_size  = best_mapping.M * best_mapping.K * 12;
            int dram_weight_size = best_mapping.K * best_mapping.N * 192;
            int dram_opsum_size  = B * best_mapping.N * 16;
            ...
            outfile << ".set DRAM_IFMAP_SIZE, " << dram_ifmap_size << "\t# M * K * 12" << endl;
            outfile << ".set DRAM_WEIGHT_SIZE, " << dram_weight_size << "\t# K * 3 * N * 4 * 16" << endl;
            outfile << ".set DRAM_OPSUM_SIZE, " << dram_opsum_size << "\t# B * N * 16" << endl;
            outfile << endl;
