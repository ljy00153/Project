...
class EyerissAnalyzer_base
{
    public:
        ...
        virtual void set_hardware(const EyerissHardwareParam& param)
        {
            hardware_param = param;
        }

        virtual int in_features_used()
        {
            return mapping.tk * 3 * DATA_SIZE;
        }

        virtual int weight_used()
        {
            return mapping.tn * mapping.tk * 12 * DATA_SIZE;
        }

        virtual int psum_used()
        {
            return mapping.tn * 4 * PSUM_DATA_SIZE;
        }

        virtual vector<pair<string, int>> glb_usage_per_pass() //OS 
        {
        ...
        virtual bool glb_size_legal()
        {
            return glb_usage_per_pass()[3].second <= 64 * 1024; // 64KB
        }

        virtual vector<pair<string,long long  int>> dram_access_per_layer()
        {
        ...
        virtual vector<pair<string, long long int>> glb_access_per_layer()
        {
            vector<pair<string, long long int>> res;
        ...
        virtual double latency_per_layer()
        {
        ...
        virtual int macs_per_layer()
        {
            return linear_shape.B * linear_shape.in_features * linear_shape.out_features;
        }

        virtual vector<pair<string, double>> energy_per_layer()
        {
        ...
        virtual vector<pair<string, double>> power_per_layer()
        {
            vector<pair<string, double>> res;
        ...
        virtual double operational_intensity()
        {
            return double(macs_per_layer()) / double(dram_access_per_layer()[6].second);
        }

        virtual int peak_performance()
        {
            return 48;
        }

        virtual int peak_bandwidth()
        {
            return hardware_param.bus_bw;
        }

        virtual string bound_by()
        {
        ...
        virtual bool is_compute_bound()
        {
            return bound_by() == "compute";
        }

        virtual bool is_memory_bound()
        {
            return bound_by() == "memory";
        }

        virtual bool is_balanced()
        {
            return bound_by() == "balanced";
        }

        virtual AnalysisResult summary()
        {
