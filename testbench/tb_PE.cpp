...
int main(int argc, char** argv) {
    // print_config();
    // Load test data
    vector<vector<uint8_t>> ifmap_data;
    vector<vector<vector<int8_t>>> filter_data;
    vector<vector<int>> ipsum_data;
    vector<vector<int>> opsum_data;
    vector<vector<int>> opsum_data_golden;

    if (!Layer) {
        ifmap_data.assign(IFMAP_COL, vector<uint8_t>(I_CH));
        filter_data.assign(OFMAP_CH, vector<vector<int8_t>>(3, vector<int8_t>(I_CH)));
        ipsum_data.assign(OFMAP_COL, vector<int>(OFMAP_CH));
        opsum_data.assign(OFMAP_COL, vector<int>(OFMAP_CH));
        opsum_data_golden.assign(OFMAP_COL, vector<int>(OFMAP_CH));
    } else {
        ifmap_data.assign(Batch * IFMAP_COL, vector<uint8_t>(I_CH));
        filter_data.assign(OFMAP_CH, vector<vector<int8_t>>(3, vector<int8_t>(I_CH)));
